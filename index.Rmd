...
In future versions we may open a tab to allow people to submit relevant papers for us to incorporate in our review.
...
We also identify the primary data extractor to allow checking and quality control.

```{r tail_study_details}
flextable(tail(data_study_general))
```
...
As a sense check this line pulls in the defined current version, which is: Version **`r paste(current_version)`**.
...
```
* **Number of studies reporting current smokers:**`r paste(nrow(current_smok))`
...
```
* **Former smokers:**`r paste(nrow(former_smok))`
...
```
* **Never smokers:**`r paste(nrow(never_smok))`
...
```
* **Current/former smokers:**`r paste(nrow(current_former_smok))`
...
```
* **Missing, never/unknown and not stated:**`r paste(nrow(smok_not_stated))`
...
If this is the first time generating this report the bootstrapping script `olgas_script.R` needs to be run in order to generate the 95% confidence intervals around the point estimates from the included studies. If this version has previously been produced then these data will be imported from the `country_prevalence.rds` in the `smoking_plots.R` script which will run here.
